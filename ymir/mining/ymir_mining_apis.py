    ...
    for idx, batch in enumerate(pbar):
        ...
        for inner_idx, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                masks_prob = torch.mean(masks_prob,0)
                masks_prob = masks_prob.data.cpu().numpy()
                uncertainty = -np.sum( masks_prob * np.log2(masks_prob + EPS)+(1 - masks_prob) * np.log2(1 - masks_prob + EPS))
                mining_results[image_file] = uncertainty       
            else:
