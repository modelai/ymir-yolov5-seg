...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    data_dict = None
    if RANK in {-1, 0}:
        logger = GenericLogger(ymir_cfg,opt=opt,console_logger=LOGGER)
    ...
    # Resume
    best_fitness, start_epoch,best_boxAP = 0.0, 0, 0.0
    if pretrained:
        if resume:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ ,box_tp_fp, mask_tp_fp= validate.run(data_dict,
            ...
            if (not nosave) or (best_fitness == fi) or (final_epoch and not evolve):  # if save
                ...
                if opt.save_period > 0 and final_epoch:
                    torch.save(ckpt, w / f'last.pt')
                    logger.log_model(w / f'last.pt')
                    weight_file = str(w / f'last.pt')
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ ,box_tp_fp,mask_tp_fp= validate.run(
                        data_dict,
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
        ...
        if plots:
            plot_results_with_masks(file=Path(ymir_cfg.ymir.output.tensorboard_dir) / 'results.csv')  # save results.png
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not opt.evolve:  # resume from specified or most recent last.pt
        last = Path(check_file(opt.resume) if isinstance(opt.resume, str) else get_latest_run(ymir_cfg.ymir.input.root_dir))
